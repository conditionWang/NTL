...
from utils_cifar_stl import add_watermark, get_augment_data, get_cifar_data, get_stl_data
...
def train():
    ...
    dataset_a = get_augment_data('cifar')
    print('augment data loaded...')

    data_a_patch = add_watermark(get_augment_data('cifar'), 32)
    data1_patch = add_watermark(get_cifar_data(), 32)
    data2_patch = add_watermark(get_stl_data(), 96)
    print('patched data loaded...')

    datasets = [data1_patch, data2_patch, dataset1, dataset2]
    
    datafile = Cus_Dataset(mode = 'authorized_training', \
                            dataset_1 = data1_patch, begin_ind1 = 1000, size1 = 6000,\
                            dataset_2 = dataset_a, begin_ind2 = 0, size2 = 0,\
                            dataset_3 = data_a_patch, begin_ind3 = 0, size3 = 3000,\
                            dataset_4 = dataset1, begin_ind4 = 1000, size4 = 3000,\
    ...
    for epoch in range(nepoch):
        ...
        for j in range(4):
            ...
            if j == 1:
                print('-'*40)
    # torch.save(model.state_dict(), 'model_{}.pth'.format('stl'))
    print('store model ok ...')
