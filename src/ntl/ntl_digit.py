...
from utils_digit import get_mnist_data, get_syn_data, get_mnist_m_data, get_svhn_data, get_usps_data, add_watermark, get_augment_data
...
def train():
    ...
    dataset5 = get_syn_data()
    print('original data loaded...')
    
    dataset_aug = get_augment_data('mnist_2')
    print('augment data loaded...')

    data_aug_patch = add_watermark(get_augment_data('mnist_2'))
    data1_patch = add_watermark(get_mnist_data())
    data2_patch = add_watermark(get_usps_data())
    data3_patch = add_watermark(get_svhn_data())
    data4_patch = add_watermark(get_mnist_m_data())
    data5_patch = add_watermark(get_syn_data(), 32, 80)
    print('patched data loaded...')

    datasets = [data1_patch, data2_patch, data3_patch, data4_patch, data5_patch, dataset1, dataset2, dataset3, dataset4, dataset5]


    # print('length of source data is {0}'.format(len(dataset1[0])))
    # print('length of target is {0}'.format(len(dataset2[0])))

    
    datafile = Cus_Dataset(mode = 'authorized_training', \
                            dataset_1 = dataset1, begin_ind1 = 1000, size1 = 5000,\
                            dataset_2 = dataset_aug, begin_ind2 = 0, size2 = 5000,\
                            dataset_3 = dataset5, begin_ind3 = 1000, size3 = 0,\
                            dataset_4 = data_aug_patch, begin_ind4 = 0, size4 = 0,\
    ...
    valloader = []
    for i in datasets:
        datafile_val = Cus_Dataset(mode = 'val', dataset_1 = i, begin_ind1 = 0, size1 = 1000)
        loader_temp = DataLoader(datafile_val, batch_size=batch_size, shuffle=True, num_workers=workers, drop_last=True)
        valloader.append(loader_temp)
    ...
    for epoch in range(nepoch):
        ...
        #test
        for j in range(10):
            acc = validate_class(valloader[j], model, epoch, num_class=num_classes)
            if j == 4:
                print('-'*40)
    # torch.save(model.state_dict(), 'model_{}.pth'.format('mnist_m'))
    # print('save model ok...')
