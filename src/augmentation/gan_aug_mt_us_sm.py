...
parser.add_argument("--data", type=str, default='usps', help="source domain")
...
for aug_index in range(opt.aug_number):
    ...
    for d in range(opt.direction):
        if opt.model_load:
            generator_aug.load_state_dict(torch.load('generator_%s.pth' % 'usps_2'))
            discriminator_aug.load_state_dict(torch.load('discriminator_%s.pth' % 'usps_2'))
            generator_aug.cuda()
