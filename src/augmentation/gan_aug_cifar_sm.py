...
from return_dataset import return_cifar10
...
parser.add_argument("--n_epochs_aug", type=int, default=1, help="number of epochs of augmentation")
...
parser.add_argument("--after_size", type=int, default=2, help="size of each image dimension")
parser.add_argument("--channels", type=int, default=3, help="number of image channels")
parser.add_argument("--sample_interval", type=int, default=100, help="interval between image sampling")
parser.add_argument("--direction", type=int, default=4, help="the direction of augmentation")
parser.add_argument("--source", type=str, default='cartoon', help="source domain")
parser.add_argument("--data", type=str, default='cifar', help="source domain")
parser.add_argument("--aug_number", type=int, default=4, help="source domain")
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        # Output layers
        self.adv_layer = nn.Sequential(nn.Linear(512 * ds_size ** 2, 256),
                        nn.ReLU(True),
                        nn.Dropout(),
                        nn.Linear(256, 128),
                        nn.ReLU(True),
        ...
        self.aux_layer = nn.Sequential(nn.Linear(512 * ds_size ** 2, 256),
                        nn.ReLU(True),
                        nn.Dropout(),
                        nn.Linear(256, 128),
                        nn.ReLU(True),
...
categorical_loss = torch.nn.KLDivLoss()
...
dataloader = return_cifar10(opt)
# dataloader = return_syn_sign(opt)
...
# optimizer_info = torch.optim.Adam(generator.parameters(), lr=opt.lr, betas=(opt.b1, opt.b2))
    ...
    for name, child in model.named_children():
        ...
        for param in child.parameters():
            if param.size(0) % 4 == 0:
                ...
                # print(inter)
                # print('freeze following different direction')
...
for epoch in range(opt.n_epochs):
    for i, (imgs, labels) in enumerate(dataloader):
        ...
        # label_gen = np.random.randint(0, opt.n_classes, batch_size)
        label_input = to_categorical(labels.numpy(), num_columns=opt.n_classes)
        # print(label_input)
        ...
        _, validity, cls_gen = discriminator(gen_imgs)
        fake_cls_loss = categorical_loss(F.log_softmax(cls_gen, dim=1), label_input)
        ...
        class_loss = categorical_loss(F.log_softmax(pred_label, dim=1), label_input)
        ...
        d_loss = para_d * d_advloss + class_loss * 10.0
        d_loss.backward()
        optimizer_D.step()
        ...
        info_loss = categorical_loss(F.log_softmax(pred_label, dim=1), label_input)
        ...
        if batches_done % (opt.sample_interval * 1) == 0:
            print("[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f] [Fake cls loss: %f] [Real cls loss: %f] [Info loss: %f]"
            % (epoch, opt.n_epochs, i, len(dataloader), d_loss.item(), g_loss.item(), fake_cls_loss.item(), class_loss.item(), info_loss.item()))
        if batches_done % (opt.sample_interval * 5) == 0:
...
for aug_index in range(opt.aug_number):
    ...
    aug_dis = 0.1 + aug_index * 0.1
    # aug_dis = 0.1
    for d in range(opt.direction):
        if opt.model_load:
            generator_aug.load_state_dict(torch.load('generator_%s.pth' % opt.data))
            discriminator_aug.load_state_dict(torch.load('discriminator_%s.pth' % opt.data))
            generator_aug.cuda()
        ...
        for epoch in range(opt.n_epochs_aug):
            ...
            for i, (imgs, labels) in enumerate(dataloader):
                ...
                # print(mmd_loss)
                class_loss = restrain_loss(F.log_softmax(gen_pred_label, dim=1), real_labels) * 0.5
                domain_loss = adversarial_loss(gen_valid, valid) * 0.5

                g_loss = class_loss - mmd_loss
                # g_loss = - mmd_loss
        ...
        for i in range(20):
            label = save_image_aug(generator_aug, (aug_index*20 + i)*opt.batch_size)
