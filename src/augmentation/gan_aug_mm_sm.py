...
from return_dataset import return_mnist_m
...
parser.add_argument("--data", type=str, default='mnist_m', help="source domain")
...
for epoch in range(opt.n_epochs):
    for i, (imgs, labels) in enumerate(dataloader):
        ...
        optimizer_D.zero_grad()
        if d_advloss.item() > 0.3 or g_loss.item() < 0.2:
            para_d = 1.0
        else:
            para_d = 0
    ...
    for d in range(opt.direction):
        ...
        for epoch in range(opt.n_epochs_aug):
            ...
            for i, (imgs, labels) in enumerate(dataloader):
                ...
                # Adversarial ground truths
                valid = Variable(FloatTensor(batch_size, 1).fill_(1.0 - aug_dis), requires_grad=False)
                fake = Variable(FloatTensor(batch_size, 1).fill_(0.0), requires_grad=False)
