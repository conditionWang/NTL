...
parser.add_argument("--data", type=str, default='svhn', help="source domain")
...
for aug_index in range(opt.aug_number):
    ...
    for d in range(opt.direction):
        if opt.model_load:
            generator_aug.load_state_dict(torch.load('generator_%s.pth' % 'svhn'))
            discriminator_aug.load_state_dict(torch.load('discriminator_%s.pth' % 'svhn'))
            generator_aug.cuda()
