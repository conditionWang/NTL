...
from return_dataset import return_visda
...
parser.add_argument("--after_size", type=int, default=7, help="size of each image dimension")
parser.add_argument("--channels", type=int, default=3, help="number of image channels")
parser.add_argument("--sample_interval", type=int, default=50, help="interval between image sampling")
parser.add_argument("--direction", type=int, default=0, help="the direction of augmentation")
parser.add_argument("--source", type=str, default='train', help="source domain")
parser.add_argument("--data", type=str, default='visda', help="source domain")
...
dataloader = return_visda(opt)
# dataloader = return_syn_sign(opt)

# Optimizers
optimizer_G = torch.optim.Adam(generator.parameters(), lr=opt.lr * 2.0, betas=(opt.b1, opt.b2))
...
for epoch in range(opt.n_epochs):
    for i, (imgs, labels) in enumerate(dataloader):
        ...
        d_loss = (d_real_loss + d_fake_loss) / 2 + class_loss * 1.0
        ...
        if batches_done % (opt.sample_interval * 25) == 0:
...
for aug_index in range(opt.aug_number):
    ...
    for d in range(opt.direction):
        if opt.model_load:
            generator_aug.load_state_dict(torch.load('generator_%s.pth' % 'visda'))
            discriminator_aug.load_state_dict(torch.load('discriminator_%s.pth' % 'visda'))
            generator_aug.cuda()
        ...
        for i in range(50):
            label = save_image_aug(generator_aug, (aug_index*50 + i)*opt.batch_size)
