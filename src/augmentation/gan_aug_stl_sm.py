...
from return_dataset import return_stl
...
parser.add_argument("--data", type=str, default='stl', help="source domain")
...
for aug_index in range(opt.aug_number):
    ...
    for d in range(opt.direction):
        if opt.model_load:
            generator_aug.load_state_dict(torch.load('generator_%s.pth' % 'stl'))
            discriminator_aug.load_state_dict(torch.load('discriminator_%s.pth' % 'stl'))
            generator_aug.cuda()
